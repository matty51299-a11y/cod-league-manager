...
## Update 2026-07-24 (Historical Dynasty — event completion popup fix)
- **Bug:** In Cod Dynasty (open-circuit) careers, some events completed without showing the final placings popup/table. Root cause was **not** Champs-specific: the batch quick-sim path (`advanceOpenCircuitEvent` → `simulateOpenCircuitSeason`) rolls past events with an insufficient field (skips) and completed whatever came next — including **interactive** events (Open LAN / World Championship / Invitational / Regional). Interactive events are only ever given a completion popup by the live `CircuitTournamentOverlay` (which requires `state.circuitTournament`), so a batch-swept interactive event finished silently with no popup. Reproduced on `gfinity_g3`, `egl_star_series`, `lvp_final_cup_s6`; Champs hit the same mechanism when the cups before it all skip.
- **Fix:** Interactive events are now never completed by the batch path. `competitionProfiles.js` is the single source of truth for `isInteractiveCircuitEvent` (re-exported from `circuitTournament.js`). `simulateOpenCircuitSeason` gained `stopBeforeInteractive` (records intervening skips, then stops before the interactive event); `advanceOpenCircuitEvent` + `simCircuitToNextMajor` pass it. The reducer extracted `startCircuitLive` / `maybeStartCircuitLive` so `SIM_NEXT_CIRCUIT_EVENT`, `SIM_CIRCUIT_TO_MAJOR` and `START_CIRCUIT_EVENT` all open the live tournament when they land on an interactive event — the popup no longer depends on one button path.
- **Unified completion object:** both `finalizeCircuitTournament` (live) and `assembleOpenCircuit` (batch) now emit the same shape — `eventId/eventName/eraId/gameTitle/eventTier/eventType/status/championTeamId/championTeamName/placements/finalPlacements/userTeamId/userPlacement/userProPointsEarned/proPointsAwarded/completedMatches/completedOrder/summaryReady` — with a safe champion+placements fallback so a completed event is never blank. `CircuitMatchOverlay` (batch reveal) now returns null during a live tournament and for interactive-event results, preventing duplicate/wrong popups.
- **Diagnostics:** added `scripts/diagnoseEventCompletionPopups.mjs` (primary), `diagnoseHistoricalEventStability.mjs`, `diagnoseEventReadability.mjs`, `diagnoseHistoricalSeasonFlow.mjs`, `diagnoseModernCdlMode.mjs`, shared `historicalCircuitHarness.mjs`, and `data/research/event_completion_bug_report.md`. `npm run build` ✓; existing `liveDE`/`openCircuit`/`circuitTournament`/`diagnoseHistoricalDynasty`/`diagnoseDynastySimulation` ✓; modern CDL unaffected (franchised, not open-circuit).

## Update 2026-07-24 (Modern CDL — Major 4 & Champs completion popup priority)
- **Bug (Modern CDL, not Historical):** The final-placements completion popup did not appear for **Major 4** or **Champs**. The engine starts the next event the moment a major completes — Challengers Finals qualifier after Major 4 (`phase → challengerQualifier`), ESWC after Champs (`beginEswc` → `phase → "major"` idx 5). The `ChallengerQualifierOverlay` rendered over the Major 4 champion screen, and the Champs champion-screen test (`!isMajorPhase && completed`) went false once the phase flipped back to "major" for ESWC, so the ESWC entry gate showed instead.
- **Fix (display-layer only; engine routing unchanged so headless sims + Historical Dynasty are unaffected):** Extended the completion-popup-priority pattern that `SeasonAwardsOverlay` already used. `MajorTournamentOverlay` champion screen now shows on `completed && !showLive` (shows Champs popup despite ESWC phase). `MajorEntryOverlay` and `ChallengerQualifierOverlay` now yield (`return null`) while an entered, completed major's popup is pending, so the next event's UI is suppressed until Continue (`DISMISS_MAJOR`). The suppressed next event does not auto-play. Added `buildCdlEventCompletionSummary()` → `schedule.lastCompletedCdlEvent` (unified `{eventId,eventName,eventType,mode:"modern_cdl",status,championTeamId/Name,placements,userTeamId,userPlacement,proPointsAwarded,prizeMoney,completedMatches,summaryReady}`), placements derived with the same `getMajorPlacementMap` the popup uses.
- **Diagnostics:** added `scripts/diagnoseModernCdlEventCompletion.mjs` (Majors 1–4 + Champs popup priority, unified summary, no duplicate points, Historical still starts); upgraded `scripts/diagnoseModernCdlMode.mjs` to a full-season smoke test; report `data/research/modern_cdl_event_completion_bug_report.md`. `npm run build` ✓; `diagnosePostSeasonFlow`/`diagnoseFullSeasonFlow`/`diagnoseHistoricalDynasty`/`diagnoseEventCompletionPopups` ✓. (`diagnoseHistoricalRosterImport.mjs` requested but absent; Historical smoke run via `diagnoseHistoricalDynasty.mjs`.)
